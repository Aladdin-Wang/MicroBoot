...
![Keil 未找到下载器](../../images/mklink/support/device-not-connected.png)
...
![Keil 未找到下载器](../../images/mklink/support/dap-driver.png)
...
下载器固件用于更新 MKLink 自身，V2 使用 `.rbl`，V3/V4 使用 `CHERRYUF2` 下的 `.uf2`。目标 MCU 固件是应用的 HEX/BIN，通过在线或脱机烧录写入芯片。两类文件不能混用，详见[固件升级](../flashing/firmware-upgrade.md)。
...
读取下载器固件版本和 Skill 版本。每个会话首次加载 Skill 时会重新检查公开版本，发现新版后由用户决定是否安装。桌面应用、Skill 和下载器固件分别核对，固件按对应型号的升级说明更新。
