...
本页面向 MKLink V4 的设备端 PikaPython 自动升级流程。脚本会向产品 APP 发送跳转 BootLoader 指令，再通过 YModem 传输固件。开始前必须确认目标产品已具备可恢复的 BootLoader、固件完整性校验和掉电保护。

!!! danger "先在可恢复样机验证"
    跳转协议、串口选择、波特率、固件分区或文件错误都可能使设备无法启动。不要未经验证直接用于批量设备。
...
参考[基于 CMSIS-PACK 移植教程](https://microboot.readthedocs.io/zh-cn/latest/microboot/getting-started/cmsis-pack/)使用microboot。
