...
通过Symbol table信息我们可以找到Init、UnInit、EraseSector和ProgramPage函数所在的位置。我们所需要的正是以上函数，接下来的任务只需要写一个上位机，将以上函数提取出来即可，这个工具我已经写好，如图：
