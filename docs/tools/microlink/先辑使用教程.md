...
本手册使用 HPM SDK `samples/segger_sysview/freertos` 作为演示工程，在保留 FreeRTOS/SystemView 的基础上增加 10 ms PID 速度环、RTT 状态帧和受控 RISC-V Trap。
...
## RTT 日志：HPM SDK 与 Web GUI

HPM SDK 自带 SEGGER RTT 示例，可在不占用目标 UART 的情况下输出日志。现有 SES 示例流程如下。

1. 在 HPM SDK 工具中打开 SEGGER RTT 的 `menu` 例程；

   ![HPM SDK RTT menu 例程](../../images/microlink/hpm_rtt_menu.png)

2. 使用 SES 编译工程，下载固件，并从 MAP 中确认 `_SEGGER_RTT` 控制块；

   ![SES MAP 中的 RTT 控制块](../../images/microlink/HPM_MAP.png)

3. 在 Web GUI 的“配置”页加载同一次构建的 ELF/MAP，进入“RTT View”，点击“自动搜索”后开始采集；
4. 看到持续输出的 HPM 示例日志，并确认复位后日志可以重新出现。

旧版固件也可通过下载器内部命令按地址启动 RTT，下图保留了原有验证方式。新项目优先使用 Web GUI 自动搜索，避免复制已经变化的控制块地址。

![HPM RTT 旧版命令验证](../../images/microlink/HPM_RTT.png)

HPM5301 实测 RTT 日志如下。采集时间覆盖 800/1600 rpm 阶跃，目标、反馈和输出持续变化。
...
完成基本烧录后，HPM 项目可以继续使用 MKLink 的通用观测能力：

- RTT View：查看启动日志、断言和命令行输出；
- SuperWatch：根据 ELF 符号采集控制变量，观察 PID、FOC 或状态机；
...
HPM5301 是 RISC-V，故障现场使用 `mcause/mepc/mtval/mstatus`，不能照搬 STM32 的 CFSR/HFSR。本次受控非法指令 Trap 采集到 `mcause=2`、`mepc=0x80008B1A`、`mtval=0xFFFFFFFF`，由 ELF 定位到 `trigger_illegal_instruction()` 的 `main.c:68`。
